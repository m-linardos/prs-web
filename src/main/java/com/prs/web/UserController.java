...
// First attempt at authentication method
//	@PostMapping("/Validate")	// @RequestParam must be before each field passed in
//	public JsonResponse getByUserNameAndPassword(@RequestParam String userName, @RequestParam String password) {
//		JsonResponse jr = null;
//		try {
...
//	}
	...
	public JsonResponse authenticate(@RequestBody User u) {
		JsonResponse jr = null;
		try {
			Optional<User> p = userRepository.findByUserNameAndPassword(u.getUserName(), u.getPassword());
			if (p.isPresent())
				jr = JsonResponse.getInstance(p);
			else
				jr = JsonResponse.getInstance("Username and password combination not found." );
		} catch (Exception e) {
			jr = JsonResponse.getInstance(e);
		}
		return jr;
